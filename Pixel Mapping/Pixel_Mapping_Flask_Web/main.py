import numpy as np
from flask import Flask, redirect, url_for, render_template, request, app

app = Flask(__name__)


def cal(Col, Row, lowerleft, lowerright, upperleft, upperright):
    # Input of Values
...
def home():
    if request.method == "POST":
        Col = int(request.form.get("rows"))
        Row = int(request.form.get("columns"))
        lowerleft = request.form.get("lowerleft")
        lowerright = request.form.get("lowerright")
        upperleft = request.form.get("upperleft")
        upperright = request.form.get("upperright")
        cornerpoints, fullgrid = cal(Col, Row, lowerleft, lowerright, upperleft, upperright)
        print(cornerpoints)
        print(fullgrid)
        return render_template("index.html", cornerpoints=cornerpoints, fullgrid=fullgrid)
